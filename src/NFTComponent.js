...
const NFTComponent = () => {
  useEffect(() => {
    async function SCCreateNonFungibleToken() {
      const operatorId = AccountId.fromString(process.env.REACT_APP_OPERATOR_ACCOUNT_ID);
      const operatorKey = PrivateKey.fromString(process.env.REACT_APP_OPERATOR_PRIVATE_KEY);
      const receiverId = AccountId.fromString(process.env.REACT_APP_RECEIVER_ACCOUNT_ID);
      const receiverKey = PrivateKey.fromString(process.env.REACT_APP_RECEIVER_PRIVATE_KEY);
